...
const API_TEMPLATES = {
  ...
  // 与 Codex 模式同构：不查余额、不联网、不用凭据，直接读 ~/.claude/projects/**/*.jsonl 统计 token。
  // 与 Codex 的差别：Claude Code 记录的是 API 模型名（deepseek-flash / deepseek-v4-pro…），
  // 命中下面 PRICING 价目表，所以这一模式能直接折成人民币（Codex 那边只有 token 数）。
  claude_code: {
    name: 'Claude Code（本地会话）', currency: 'CNY', keyRef: '', kind: 'claude_code',
    apiNote: '不联网、不需要密钥：直接读本机 ~/.claude/projects 的会话日志统计 token（含子代理与不在本 Harness 里跑的会话）。'
      + '日志里记的是 API 模型名，命中内置价目表 → 能按峰谷价折成人民币，比 Codex 模式多一个金额维度。',
    balance: { url: '', auth: '', json: { remaining: '' } },
  },
...
// —— Claude Code 会话日志解析（纯函数：只吃 JSONL 文本，不碰 fs / 外层作用域，便于单测）——
// 数据源：~/.claude/projects/<项目 slug>/<会话 uuid>.jsonl，一行一个事件对象。
// ⚠ 关键口径：Claude Code 会把**一次 API 响应**按 content block 拆成多条 type='assistant' 的行
//   （apiBlockIndex 0..N），每行携带逐字段完全相同的 usage。逐行求和会成倍多算
//   （实测某会话 cache_read 491M vs 实际 207M），所以必须按 message.id 去重、只取首条。
// 另：output_tokens_details.thinking_tokens 已含在 output_tokens 内（是其子集），只做展示拆分。
export function parseClaudeTranscriptLines(text) {
  const recs = []
  const seen = new Set()
  for (const line of String(text || '').split('\n')) {
    ...
    if (!model || model === '<synthetic>') continue // 合成条目（本地报错）不是真实请求，不计费
    ...
    const reason = Number((u.output_tokens_details && u.output_tokens_details.thinking_tokens) || 0) || 0
    const total = input + cached + cwrite + out
    if (total <= 0) continue
    recs.push({ ts, model, input, cached, cwrite, out, reason, total })
  }
  return recs
}
...
export default {
  ...
  apply(ctx) {
    ...
    // 两个本地会话来源的差异只有「目录 / 文件名 / 单文件解析器」，聚合口径完全一致 → 合并成一份骨架。
    const LOCAL_SESSION_SOURCES = {
      codex: {
        ...
        homeName: '.codex',
        probeSub: 'sessions',
        scanDirs: ['sessions', 'archived_sessions'],
        fileRe: /^rollout-.*\.jsonl$/i,
        cacheFile: '.dshw-codex.json',
      ...
      claude_code: {
        ...
        scanDirs: ['projects'],
        fileRe: /\.jsonl$/i,
        cacheFile: '.dshw-claude.json',
        parse: parseClaudeFile,
      },
    }
    function localSessionHome(src) {
      const env = String(process.env[src.envVar] || '').trim()
      for (const c of [env, path.join(os.homedir(), src.homeName)]) {
        try {
          if (!c) continue
          // 光是目录存在还不够：得是存过会话的那一层（probeSub），
          // 否则「装了 Claude Code 但没跑过」会被当成有效来源，显示成全 0。
          if (fs.existsSync(path.join(c, src.probeSub))) return c
        } catch (err) {}
      }
      return ''
    }
    function readLocalCache(src) {
      try {
        const j = JSON.parse(fs.readFileSync(path.join(DSH_HOME, src.cacheFile), 'utf8'))
        if (j && j.files && typeof j.files === 'object') return j
      } catch (err) {}
      return { version: 1, files: {} }
    }
    function writeLocalCache(src, c) {
      try { fs.writeFileSync(path.join(DSH_HOME, src.cacheFile), JSON.stringify(c), 'utf8'); return true } catch (err) { return false }
    }
    function listLocalSessionFiles(src, root) {
      const out = []
      const walk = (dir, depth) => {
        ...
        for (const e of ents) {
          ...
          else if (src.fileRe.test(e.name)) out.push(p)
        }
      }
      for (const d of src.scanDirs) walk(path.join(root, d), 0)
      return out
    }
    // day → model → 计数量的累加器；Codex / Claude Code 两个解析器共用。
    // v.pk（1=高峰价 0=谷价）只有 Claude Code 解析器会给：把计费相关的四个量另存一份按峰谷分开的副本，
    // 价格留到汇总时才代入，缓存里不冻结单价（用户改自定义单价后无需重新解析即可生效）。
    function makeDayBump(days) {
      return (day, mk, v) => {
        days[day] = days[day] || {}
        const cur = days[day][mk] || {
          in: 0, cached: 0, cwrite: 0, out: 0, reason: 0, total: 0, turns: 0,
          peak: null, off: null,
        }
        cur.in += Number(v.in) || 0; cur.cached += Number(v.cached) || 0; cur.cwrite += Number(v.cwrite) || 0
        cur.out += Number(v.out) || 0; cur.reason += Number(v.reason) || 0
        cur.total += Number(v.total) || 0; cur.turns += Number(v.turns) || 0
        if (v.pk === 0 || v.pk === 1) {
          const b = cur[v.pk ? 'peak' : 'off'] || (cur[v.pk ? 'peak' : 'off'] = { in: 0, cached: 0, cwrite: 0, out: 0 })
          b.in += Number(v.in) || 0; b.cached += Number(v.cached) || 0
          b.cwrite += Number(v.cwrite) || 0; b.out += Number(v.out) || 0
        }
        days[day][mk] = cur
      }
    }
    // 一个「日 + 模型」桶折算成人民币。峰谷分别按各自档位计，公式与账本里的每轮结算一致
    // （见 appendUsageEvent 上游的 assistant/message 处理）：缓存命中按命中价，输入与缓存写入按未命中价。
    // Codex 的桶没有 peak/off 副本 → 恒为 0（Codex 只统计 token，不算钱）。
    // ponytail: 缓存写入与输入同档；官方若单独给 cache write 定价，这里要加一档。
    function localBucketCost(model, v) {
      const p = priceFor(model)
      let cost = 0
      for (const slot of ['off', 'peak']) {
        const b = v && v[slot]
        if (!b) continue
        const off = slot === 'peak' ? 1 : 0
        cost += (Number(b.cached) || 0) / 1e6 * p.hit[off]
          + ((Number(b.in) || 0) + (Number(b.cwrite) || 0)) / 1e6 * p.miss[off]
          + (Number(b.out) || 0) / 1e6 * p.out[off]
      }
      // 自定义单价若按美元填写，按用户填的汇率换算成人民币（账本统一按 CNY）
      try {
        const meta = customPriceMetaFor(model)
        if (meta && meta.cur === 'USD' && Number(meta.rate) > 0) cost = cost * Number(meta.rate)
      } catch (err) {}
      return cost
    }
    ...
    // ===== Claude Code 本地会话统计 =====
    // 数据源：~/.claude/projects/<项目 slug>/<会话 uuid>.jsonl（根目录可用 $CLAUDE_CONFIG_DIR 覆盖）
    // 逐行解析与去重口径在 parseClaudeTranscriptLines（模块顶层，可单测），这里只做「按天 + 峰谷」归档。
    function parseClaudeFile(file) {
      const days = {}
      let text = ''
      try { text = fs.readFileSync(file, 'utf8') } catch (err) { return { days } }
      const bump = makeDayBump(days)
      for (const r of parseClaudeTranscriptLines(text)) {
        // 只记「量」和峰谷归属，价格留到汇总时再代入（见 localBucketCost）——
        // 这样缓存里不冻结单价，用户改自定义单价后立刻生效。
        bump(dayKeyFromTs(r.ts), r.model, {
          in: r.input, cached: r.cached, cwrite: r.cwrite, out: r.out, reason: r.reason,
          total: r.total, turns: 1,
          pk: isPeakTime(Math.floor(r.ts / 1000)) ? 1 : 0,
        })
      }
      return { days }
    }
    ...
    function localSessionSummary(kind) {
      // 汇总时要用当前的自定义单价换算金额（10 秒节流），否则后台预热那次会用内置价表算出结果
      try { refreshCustomPrices() } catch (err) {}
      const src = LOCAL_SESSION_SOURCES[kind]
      if (!src) return { ok: false, error: '未知的本地会话来源：' + kind }
      const home = localSessionHome(src)
      if (!home) return { ok: false, error: '未找到 ' + src.label + ' 目录（$' + src.envVar + ' 或 ~/' + src.homeName + '）' }
      const cache = readLocalCache(src)
      const files = listLocalSessionFiles(src, home)
      const keep = {}
      let changed = 0
      ...
      if (changed > 0 || Object.keys(cache.files).length !== Object.keys(keep).length) {
        writeLocalCache(src, { version: 1, files: keep, builtAt: Date.now() })
      }
      ...
      let totalCost = 0, todayCost = 0, monthCost = 0
      ...
      for (const f of Object.keys(keep)) {
        ...
        for (const d of Object.keys(days)) {
          const day0 = byDay[d] || (byDay[d] = { tokens: 0, turns: 0, cost: 0, models: {} })
          for (const m of Object.keys(days[d])) {
            const v = days[d][m] || {}
            const cost = localBucketCost(m, v)
            day0.tokens += Number(v.total) || 0
            day0.turns += Number(v.turns) || 0
            day0.cost += cost
            day0.models[m] = (day0.models[m] || 0) + (Number(v.total) || 0)
            const bm = byModel[m] || (byModel[m] = { tokens: 0, out: 0, reason: 0, cached: 0, turns: 0, cost: 0 })
            ...
            totalCost += cost
            if (d === today) { todayTokens += Number(v.total) || 0; todayCost += cost }
            if (d.slice(0, 7) === month) { monthTokens += Number(v.total) || 0; monthCost += cost }
          }
        }
      }
      const days7 = []
      for (let i = 0; i < 7; i++) {
        ...
        days7.push({
          date: d,
          tokens: (byDay[d] && byDay[d].tokens) || 0,
          turns: (byDay[d] && byDay[d].turns) || 0,
          cost: preciseMoney(Number(byDay[d] && byDay[d].cost) || 0),
        })
      }
      return {
        ok: true, kind, label: src.label, home, sessions: files.length, changed,
        todayTokens, monthTokens, totalTokens,
        outTokens, reasonTokens, cachedTokens,
        // 金额一律走 accounting.mjs 的精确分单位（v0.3.1 起取代了原来的 round2）
        todayCost: preciseMoney(todayCost),
        monthCost: preciseMoney(monthCost),
        totalCost: preciseMoney(totalCost),
        days7, byModel,
    ...
    function codexSummary() { return localSessionSummary('codex') }
    function claudeSummary() { return localSessionSummary('claude_code') }
    ...
    let claudeMemo = null, claudeMemoAt = 0
    function claudeSummaryCached() {
      const now = Date.now()
      if (claudeMemo && now - claudeMemoAt < 5000) return claudeMemo
      claudeMemo = claudeSummary()
      claudeMemoAt = now
      return claudeMemo
    }
    // 按模板 kind 取对应的本地会话统计（Codex / Claude Code 共用同一套「本地会话」口径）
    function localSummaryForKind(kind) {
      return kind === 'claude_code' ? claudeSummaryCached() : codexSummaryCached()
    }
    // 本地会话来源的两个 kind（作模板判断用，避免到处散落字符串比较）
    function isLocalSessionKind(kind) { return kind === 'codex' || kind === 'claude_code' }
    // 额度面板里「已用来源 = 本地会话」的持久化取值。沿用历史字符串 'codex' 以免迁移用户已存的配置，
    // 语义已泛化为「本地会话 token」；到底读 Codex 还是 Claude Code 由模型模板的 kind 决定。
    const LOCAL_SESSION_MODE = 'codex'
    // 某个注册模型对应的本地会话来源 kind
    function localKindOfModel(id) {
      try {
        const m = apiModelById(id)
        return ((m && apiTemplateOf(m.provider)) || {}).kind || 'codex'
      } catch (err) { return 'codex' }
    }
    ...
    try {
      const warmLocal = function () {
        try { codexSummaryCached() } catch (err) {}
        try { claudeSummaryCached() } catch (err) {}
      }
      setTimeout(warmLocal, 1500)
      setInterval(warmLocal, 5 * 60 * 1000)
    } catch (err) {}
    // ===== 自定义 API 模型注册表（v655）=====
    ...
    async function apiProbeModel(model) {
      ...
      // 本地会话模式（Codex / Claude Code）：本地统计即「探活结果」，不需要网络与密钥
      if (isLocalSessionKind(tpl.kind)) {
        const cs = localSummaryForKind(tpl.kind)
        const f = (n) => (n >= 100000000 ? (n / 100000000).toFixed(2) + '亿' : (n >= 10000 ? (n / 10000).toFixed(1).replace(/\.0$/, '') + '万' : String(Math.round(Number(n) || 0))))
        const label = (cs && cs.label) || '本地会话'
        if (cs && cs.ok) {
          return { ok: true, detail: label + ' 本地会话：今日 ' + f(cs.todayTokens) + ' · 本月 ' + f(cs.monthTokens) + ' · 累计 ' + f(cs.totalTokens) + ' tokens（' + cs.sessions + ' 个会话文件）' }
        }
        return { ok: false, error: (cs && cs.error) || '未找到本地会话目录' }
      }
      let key = ''
      try {
    ...
    function apiQuotaAutoUsed(id, q) {
      ...
      if (q && q.unit === 'money' && (q.mode === 'auto' || q.mode === LOCAL_SESSION_MODE)) {
        return Math.round(Number(q.used) || 0)
      }
      // 本地会话模式：已用 = 本地会话统计的 token（按重置口径取今日/本月/累计）。
      // 取值 'codex' 是历史遗留，语义已泛化为「本地会话 token」；具体读 Codex 还是 Claude Code
      // 由该模型的模板 kind 决定（见 LOCAL_SESSION_SOURCES）。
      if (q && q.mode === LOCAL_SESSION_MODE) {
        const cs = localSummaryForKind(localKindOfModel(id))
    ...
    async function apiModelsPayload() {
      ...
      const out = []
      const localStats = {} // 本地会话统计（机器级；每个来源算一次，多个同来源模型共享）
      for (const m of models) {
        ...
        if (m.id === API_BUILTIN_ID) {
          ...
          // 本地会话模式（Codex / Claude Code）：不查余额、不需要密钥，直接给本地统计（今日/本月/累计/近7天）
          if (isLocalSessionKind(tplM.kind)) {
            if (!localStats[tplM.kind]) localStats[tplM.kind] = localSummaryForKind(tplM.kind)
            const ls = localStats[tplM.kind]
            // 字段名沿用 codex（前端与泡泡模块已按它取值），Claude Code 走同名字段
            entry.codex = ls
            entry.hasKey = true
            entry.error = ls && ls.ok ? null : ((ls && ls.error) || '未找到本地会话目录')
          }
